...
namespace DemoInstagram.APIsHelper
{
    ...
    {
        private readonly IBusiness _instagramBussiness;

        public EndpointImpl(IBusiness instagramBussiness)
        {
            ...
            using (var client = new HttpClient())
            {
                using (var response = await client.GetAsync(path))
                {
                    ...
                    else
                    {
                        ...
                        throw new Exception(_instagramBussiness.ProcessError(response));
                    }
                    return profile;
                }
            }
        ...
        public async Task<List<Comment>> LoadComments(string imageId)
        {
            ...
            using (var client = new HttpClient())
            {
                using (var response = await client.GetAsync(path))
                {
                    ...
                    {
                        //Process error json from server
                        throw new Exception(_instagramBussiness.ProcessError(response));
                    }
                    return listComment;
                }
